"""
Table 6: paired surrogate-fitness comparison, baseline vs My Method (BLX-alpha
+ tournament-k3-top20 + elitism + adaptive mutation), same seed/Hamiltonian.

For each pair, the SAME random state is used to run both baseline and My
Method, so the only thing differing between the two runs in a pair is the
algorithm itself -- this is what makes the pairing valid for Wilcoxon / Vargha-
Delaney A12 later. "Fitness" here is the raw XGBoost surrogate score
(get_y_prediction's y_pred), re-evaluated once on each run's returned vector --
never the validated eigensolver energy.
"""
import sys
import random
import csv
...
from kcl_tests_adapt_vqe import test_static_adapt, test_semi_dynamic_adapt
...
for trial in range(n_pairs):
    ...
    b_res = base.opt_hyperparams(
        model_file, generate_hyper_params_avqe, xgboost.XGBRegressor,
        n_qubits, ham, ham28_vec, max_size,
        test_static_adapt, test_semi_dynamic_adapt, None)
    ...
    # advance to a genuinely different state for the next trial
    random.seed()
    np.random.seed()
